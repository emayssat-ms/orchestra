...
SSH_KEYPAIR_PEM?=$(SSH_KEYPAIR_DIR)/$(SSH_KEY_NAME).pem
SSH_KEYPAIR_PUB?=$(SSH_KEYPAIR_PEM).pub
SSH_KNOWN_HOSTS?=localhost 127.0.0.1
# SSH_HOST_KEY_CHECKING?=true
SSH_LABEL?=[ssh] #
# SSH_OPTIONS?= -v

ifeq ($(SSH_HOST_KEY_CHECKING), false)
  __SSH_OPTIONS+= -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no
endif

ifneq ($(SSH_REMOTE_USER),)
  __SSH_OPTIONS+= -l $(SSH_REMOTE_USER)
endif


SSH?=ssh $(__SSH_OPTIONS) $(SSH_OPTIONS)
SCP?=scp -i $(SSH_KEYPAIR_PEM) $(__SSH_OPTIONS) $(SSH_OPTIONS)
...
_ssh_view_makefile_variables:
	...
	@echo " C  SSH_KEYPAIR_PEM=$(SSH_KEYPAIR_PEM)"
	@echo " C  SSH_KEYPAIR_PUB=$(SSH_KEYPAIR_PUB)"
	@echo
...
_ssh_execute:
	...
	$(SSH) $(SSH_REMOTE_HOST) $(SSH_REMOTE_COMMAND)
